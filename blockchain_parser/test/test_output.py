...
from blockchain_parser.output import TxOutput


class TestOutput(unittest.TestCase):
    def test_pubkeyhash_from_bytes(self):
        raw_output = "01000000000000001976a91432ba382cf668657bae15ee0a97fa87" \
                     "f12e1bc89f88ac"
        output = TxOutput.from_bytes(a2b_hex(raw_output))
        self.assertTrue(output.script.is_pubkeyhash())
        self.assertEqual("pubkeyhash", output.script_type)
        self.assertEqual(1, output.value)
        self.assertEqual(1, len(output.addresses))

    def test_pubkey_from_bytes(self):
        raw_output = "0100000000000000232102c0993f639534d348e1dca30566491e6c" \
                     "b11c14afa13ec244c05396a9839aeb17ac"
        output = TxOutput.from_bytes(a2b_hex(raw_output))
        self.assertTrue(output.script.is_pubkey())
        self.assertEqual("pubkey", output.script_type)
        self.assertEqual(1, len(output.addresses))
    ...
    def test_p2sh_from_hex(self):
        ...
        self.assertTrue(output.script.is_p2sh())
        self.assertEqual("p2sh", output.script_type)
        self.assertEqual(1, len(output.addresses))
    ...
    def test_return_from_hex(self):
        ...
        output = TxOutput.from_bytes(a2b_hex(raw_output))
        self.assertTrue(output.script.is_return())
        self.assertEqual("OP_RETURN", output.script_type)
    ...
    def test_multisig_from_hex(self):
        ...
        output = TxOutput.from_bytes(a2b_hex(raw_output))
        self.assertTrue(output.script.is_multisig())
        self.assertEqual("multisig", output.script_type)
        self.assertEqual(2, len(output.addresses))

    def test_unknown_from_hex(self):
        raw_output = "01000000000000000151"
        output = TxOutput.from_bytes(a2b_hex(raw_output))
        self.assertEqual("unknown", output.script_type)
        self.assertEqual(0, len(output.addresses))
